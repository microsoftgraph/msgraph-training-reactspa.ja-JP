...
- 開発用のコンピューターにインストールされている[node.js](https://nodejs.org) 。 Node.js を持っていない場合は、「ダウンロードオプション」の前のリンクにアクセスしてください。 (**注:** このチュートリアルは、ノードバージョン12.16.1 を使用して作成されています。 このガイドの手順は、他のバージョンでは動作しますが、テストされていません。)
...
1. ファイルの`./graph-tutorial/src/Config.ts.example`名前を`./graph-tutorial/src/Config.ts`に変更します。
1. `./graph-tutorial/src/Config.ts`ファイルを編集し、次のように変更します。
